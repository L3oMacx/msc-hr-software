        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
            ...
            <form>
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="">
                        <h3>{{ $data_modal_heading }}</h3>
                        <div class="mb-4">
                            <label for="dataValue"
                                class="block text-gray-700 text-sm font-bold mb-2">Wert *</label>
                            @if ($data_key_object->input_type === 'text')
                                <input type="text"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="dataValue" placeholder="Wert eingeben" wire:model="data_value">
                            @elseif ($data_key_object->input_type === 'select')
                                <select
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                id="dataValue" wire:model="data_value">
                                    @foreach ($data_key_object->select_options as $option)
                                        <option value="{{ $option['id'] }}">{{ $option['name'] }}</option>
                                    @endforeach
                                </select>
                            @endif

                            @error('data_value') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                        <div class="mb-4">
                            ...
                            @error('data_valid_from') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                        <div class="mb-4">
                            ...
                            @error('data_valid_to') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                    </div>
                </div>
